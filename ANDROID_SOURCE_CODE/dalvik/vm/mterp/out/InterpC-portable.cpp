...
typedef int (*callback) (Thread* self, const Method* curMethod, const Method* methodToCall);

static inline int Direct_Hook(Thread* self, const Method* curMethod, const Method* methodToCall)
{
		/*the hook for Direct Method. Put your code here */

		/*
		const char * c = curMethod->name;	
		const char * tc = methodToCall->name;
		const char * s1 = "perform";
		const char * s2 = "hook_perform";
		const char * s3 = "native_window_set_buffers_geometry";
		const char * s4 = "native_window_set_buffers_dimensions";

		if(strstr(tc, s1)){
			LOGE("AMIT-DALVIK: %s invoking method containing perform", c);
		}
		
		if(strstr(tc, s2)){
			LOGE("AMIT-DALVIK: %s invoking method containing hook_perform", c);
		}
		
		if(strstr(tc, s3)){
			LOGE("AMIT-DALVIK: %s invoking method containing native_window_set_buffers_geometry", c);
		}
		
		if(strstr(tc, s4)){
			LOGE("AMIT-DALVIK: %s invoking method containing native_window_set_buffers_dimensions", c);
		}

		LOGE("AMIT-DALVIK - header.cpp: after all if blocks");
		*/

		return 1;
}

static inline int dvmInterpretHook(Thread* self, const Method* curMethod, const Method* methodToCall,callback func)
{
		/* the function is called in gotoTargets.cpp .
		 * 	 * If you want to add some code to affect all the hook, put it here */
		
	    //func(self,curMethod,methodToCall);
		...
		if(strstr(tc, s1)){
			LOGE("AMIT-DALVIK: %s invoking method containing perform : %s", c, tc);
		}
		
		if(strstr(tc, s2)){
			LOGE("AMIT-DALVIK: %s invoking method containing hook_perform : %s", c, tc);
		}
		
		if(strstr(tc, s3)){
			LOGE("AMIT-DALVIK: %s invoking method containing native_window_set_buffers_geometry : %s", c, tc);
		}
		
		if(strstr(tc, s4)){
			LOGE("AMIT-DALVIK: %s invoking method containing native_window_set_buffers_dimensions : %s", c, tc);
		}
...
GOTO_TARGET(invokeVirtual, bool methodCallRange, bool jumboFormat)
    {
		...
		//AMIT
		dvmInterpretHook(self,curMethod,methodToCall,Direct_Hook);	
		//END
