...
static void Surface_postAframeBounds(
		JNIEnv* env, jobject clazz,
		jstring activityName, jint left, jint top, jint right, jint bottom)
{
	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 0");
	const sp<Surface>& surface(getSurface(env, clazz));
	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 1");
	const jchar* str = env->GetStringCritical(activityName, 0);
	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 2");
	const String8 name(str, env->GetStringLength(activityName));
    env->ReleaseStringCritical(activityName, str);
    
	//LOGD("AMIT - POSTBOUNDS - android_view_Surface - postAframeBounds : arg - activityName(%s), left(%d), top(%d), right(%d), bottom(%d)", name.string(), left, top, right, bottom);

	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 3");
	//const sp<Surface>& surface(getSurface(env, clazz));


	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 4");
	if( (surface != 0) && (Surface::isValid(surface)) ) {
		surface->postAframeBounds(name, left, top, right, bottom);
	}

	//LOGD("AMIT - INSIDE Surface_postAframeBounds() - 5");
}
...
static jobject Surface_lockCanvas_withUid(JNIEnv* env, jobject clazz, jobject dirtyRect, jint pairUid)
{
    const sp<Surface>& surface(getSurface(env, clazz));
    if (!Surface::isValid(surface)) {
        doThrowIAE(env);
    ...
    ////////////////////////////////////////////////////
    // AMIT - added uid argument in following invocation
    
    status_t err = surface->lock(&info, pairUid, &dirtyRegion);
...
static JNINativeMethod gSurfaceMethods[] = {
    ...
    ///////////////////////////////////////////////
    // AMIT - added new overload with pair uid arg

    {"lockCanvasNative",    "(Landroid/graphics/Rect;I)Landroid/graphics/Canvas;",  (void*)Surface_lockCanvas_withUid },
	...
	{"postAframeBoundsNative", "(Ljava/lang/String;IIII)V", (void*)Surface_postAframeBounds },
