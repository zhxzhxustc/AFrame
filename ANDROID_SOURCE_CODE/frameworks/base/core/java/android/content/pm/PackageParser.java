...
public class PackageParser {
    ...
    private boolean parseApplication(Package owner, Resources res,
        ...
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
            ...
            String tagName = parser.getName();
	// XIAO: 11/25/2012
            if (tagName.equals("aframe")) {
		sa = res.obtainAttributes(attrs,com.android.internal.R.styleable.AndroidManifestAframe);
		String aframeName = sa.getNonConfigurationString(com.android.internal.R.styleable.AndroidManifestAframe_name,0);
		
		Aframe aframe = parseAframe(pkgName, aframeName, res, parser, attrs);
		owner.aframes.add(aframe);
		
		Log.d("XIAO","------------Debugging Installation----------");
   	    	Log.d("XIAO","packageName = " + pkgName);
		Log.d("XIAO","uid = " + ai.uid);
		Log.d("XIAO","ComponentName = " + aframeName);
		Log.d("XIAO","Permission = " + aframe.aframePermissions);
    ...
    // XIAO 11/28/2012
    private Aframe parseAframe(String pkgName, String aframeName, Resources res, XmlPullParser parser, AttributeSet attrs)
            throws XmlPullParserException, IOException {

	Aframe aframe = new Aframe(pkgName,aframeName);

        TypedArray sa = res.obtainAttributes(attrs,
                com.android.internal.R.styleable.AndroidManifestAframe);    
	int outerDepth = parser.getDepth();
        int type;
        while ((type=parser.next()) != XmlPullParser.END_DOCUMENT
	    ...
	    Log.d("XIAO","tagName = " + parser.getName());
            if (parser.getName().equals("aframe-permission")) {
  		sa = res.obtainAttributes(attrs,
                        com.android.internal.R.styleable.AndroidManifestAframePermission);
                String name = sa.getNonResourceString(
                        com.android.internal.R.styleable.AndroidManifestAframePermission_name);
                sa.recycle();
		aframe.aframePermissions.add(name.intern());
            }  
        }
	return aframe;
    }
    ...
    public final static class Package {
        ...
        //XIAO 11/28/2012
        public final ArrayList<Aframe> aframes = new ArrayList<Aframe>(0);
		//DONE 11/28/2012
	...
	//XIAO 11/28/2012
    public final static class Aframe {
		public String aframeName;
		public String packageName;
		public int aframeUid;
		public int appUid;
		public ArrayList<String> aframePermissions;

		public Aframe(String pkgName, String cmpName){
			aframeName = cmpName;
			packageName = pkgName;
			aframeUid = -1;
			appUid = -1;
			aframePermissions = new ArrayList<String>(0);
		}
	}
	//DONE 11/28/2012
