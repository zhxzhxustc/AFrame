...
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
	...
	/////////////////////////////////////////
	// AMIT - added stub for two cases

	case GET_AFRAME_UID_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            int uid = data.readInt();
            int res = getAframeUid(uid);
            reply.writeNoException();
            reply.writeInt(res);
            return true;
   	...
   	case GET_MAINAPP_UID_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            int uid = data.readInt();
            int res = getMainAppUid(uid);
            reply.writeNoException();
            reply.writeInt(res);
            return true;
...
class ActivityManagerProxy implements IActivityManager
{
    ...
    ///////////////////////////////////////////
    // AMIT - added proxy for two new functions

    /*
   	GET_AFRAME_UID_TRANSACTION   
   	GET_MAINAPP_UID_TRANSACTION      
    */

    public int getAframeUid(int uid) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeInt(uid);
        mRemote.transact(GET_AFRAME_UID_TRANSACTION, data, reply, 0);
        reply.readException();
        int res = reply.readInt();
        data.recycle();
        reply.recycle();
    ...
    public int getMainAppUid(int uid) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeInt(uid);
        mRemote.transact(GET_MAINAPP_UID_TRANSACTION, data, reply, 0);
        reply.readException();
        int res = reply.readInt();
        data.recycle();
        reply.recycle();
