...
public final class ViewRootImpl extends Handler implements ViewParent,
    ...
    //XIAO 11/30/2012
    private Rect mAframeContainerRect = new Rect();
    private Rect mAframeReserveRect = new Rect();
	
    private int[] mAframeContainerLocation = new int[2];
    private int[] mAframeReserveLocation = new int[2];

    private boolean hasAframeContainer = false;
    private boolean hasAframeReserve = false;

    final HashMap<String, Region> mInputChannelRegionMap = new HashMap<String, Region>();
    final HashMap<String, String> mInputChannelPkgNameMap = new HashMap<String, String>();
    final HashMap<String, String> mPkgNameInputChannelMap = new HashMap<String, String>();

    //DONE 11/30/2012

	//////////////////////////////////////////////////
	// AMIT - 04.22.2013
	// mapping between activity name and aframe bounds
	// (bounds = region that will act transparent)

	static final HashMap<String, Rect> mActivityAframeBoundsMap = new HashMap<String, Rect>();
	...
	//XIAO 11/30/2012
    private void checkAframeContainer(View view){
     	if(view.getClass().getName().equals("android.widget.AframeContainer")){
		hasAframeContainer = true;
		view.getDrawingRect(mAframeContainerRect);
	     	view.getLocationOnScreen(mAframeContainerLocation);
	}
	
	boolean isViewGroup = true;
	try{
	    	ViewGroup testGroup = (ViewGroup)view;
	}catch(Exception e){isViewGroup = false;}

	if(isViewGroup){
	     	ViewGroup group = (ViewGroup)view;
		int count = group.getChildCount();
		for(int i=0; i<count; i++){
	    	checkAframeContainer(group.getChildAt(i));
		}
	}
    }

	
    private void checkAframeReserve(View view){
	if(view.getClass().getName().equals("android.widget.AframeReserve")){
		hasAframeReserve = true;
		view.getDrawingRect(mAframeReserveRect);
    		view.getLocationOnScreen(mAframeReserveLocation);
	}
	...
	if(isViewGroup){
		...
		int count = group.getChildCount();
		for(int i=0; i<count; i++){
	    	checkAframeReserve(group.getChildAt(i));
		}
	}
    }
    ...
    private void draw(boolean fullRedrawNeeded) {
		...
		checkAframeContainer(mView);
		checkAframeReserve(mView);

		// activity layout has AframeReserve in its layout
		if(hasAframeReserve){

			String longName = mInputChannel.getName();

			//Log.d("ViewRootImpl", "AMIT - POSTBOUNDS - " + "ViewRootImpl::draw(bool) - activity (" + longName + ") layout has aframeReserve");

			String activityName = (longName.split(" "))[1];
			
			///////////////////////////////////////////////////////////
			// Use of mActivityAframeBoundsMap based on:

			/*
			Hash<activity name, aframe reserve bounds>
			static final HashMap<String, Rect> mActivityAframeBoundsMap 
											= new HashMap<String, Rect>();
			
			if activity has aframeReserve, 
							use activity name as key in hash structure

			a) if key found, compare aframe reserve bounds
					- if bounds are different , update the bounds 
					using Surface API and in hash structure

			b) if key not found
					- add bounds as value in hash structure and 
					also send bounds using Surface API
			*/
			///////////////////////////////////////////////////////////

			// calculate aframe bounds
			// (test)
			//Rect aframeRect = new Rect(100,100,600,1000);
			
			Display screenDisplay = WindowManagerImpl.getDefault().getDefaultDisplay();
			Point screenSize = new Point();
			screenDisplay.getSize(screenSize);
			int screenWidth = screenSize.x;
			int screenHeight = screenSize.y;

			//Log.d("ViewRootImpl", "ViewRootImpl::draw(bool) - ScreenDimension = [0," + screenWidth + ",0," + screenHeight + "]");

			//Log.d("ViewRootImpl", "ViewRootImpl::draw(bool) - mAframeReserverRect = " + mAframeReserveRect);
    			Rect aframeRect = mAframeReserveRect;

			aframeRect.offsetTo(mAframeReserveLocation[0],mAframeReserveLocation[1]);
			//Log.d("ViewRootImpl", "ViewRootImpl::draw(bool) - Before intersect: aframeRect = " + aframeRect);			

			if(!aframeRect.intersect(0,0,screenWidth,screenHeight)){
				//Log.d("ViewRootImpl", "View Is Out of Bound!");
				return;	
			} else{
				//Log.d("ViewRootImpl", "ViewRootImpl::draw(bool) - After intersect: aframeRect = " + aframeRect);
			}

			// check if key exists
			if( mActivityAframeBoundsMap.containsKey(activityName) ) {

			//Log.d("ViewRootImpl", "AMIT - POSTBOUNDS - mActivityAframeBoundsMap contains key: " + activityName);
			
				// get mapped Rect
				Rect mappedRect = mActivityAframeBoundsMap.get(activityName);

				// compare with new Rect
				if( ! mappedRect.equals(aframeRect) ){

					//update Rect in HashMap (sync method call)
					updateActivityAframeBoundsMap(activityName, aframeRect);

					//post new aframe bounds to surface flinger
					surface.postAframeBounds(activityName, aframeRect);
				}
			}
			else {

			//Log.d("ViewRootImpl", "AMIT - POSTBOUNDS - mActivityAframeBoundsMap doesn't contain key: " + activityName);

					//add Rect in HashMap (sync method call)
					updateActivityAframeBoundsMap(activityName, aframeRect);

					//post aframe bounds to surface flinger
					surface.postAframeBounds(activityName, aframeRect);
			}
			//Log.d("ViewRootImpl","ViewRootImpl::draw(bool) - hasAframeReserver: start handling events");
			mAframeReserveRect.offsetTo(mAframeReserveLocation[0],mAframeReserveLocation[1]);

			Region region = new Region(mAframeReserveRect);
			try {

				if(!mInputChannelPkgNameMap.containsKey(mInputChannel.getName())){
					mInputChannelRegionMap.put(mInputChannel.getName(),region);
					Display.getWindowManager().registerAppInputChannelRegion(mInputChannel, region);
				}
				if(!mPkgNameInputChannelMap.containsKey(mInputChannel.getName())){
					mInputChannelPkgNameMap.put(mInputChannel.getName(),mView.getContext().getPackageName());
					Display.getWindowManager().registerAppInputChannelPkgName(mInputChannel, mView.getContext().getPackageName());
				}
			}catch(Exception e){Log.d("ViewRootImpl","ViewRootImpl::draw(bool) - Error in registeration!!!!" + e);}
					
		} else if(hasAframeContainer){

			String longName = mInputChannel.getName();

			//Log.d("ViewRootImpl", "AMIT - POSTBOUNDS - " + "ViewRootImpl::draw(bool) - activity (" + longName + ") layout has aframeContainer");

			//Log.d("ViewRootImpl","ViewRootImpl::draw(bool) - hasAframeContainer:  start handling events");

			Region region = new Region(mAframeContainerRect);
			try {

				if(!mPkgNameInputChannelMap.containsKey(mView.getContext().getPackageName())){
					mPkgNameInputChannelMap.put(mView.getContext().getPackageName(), mInputChannel.getName());
					Display.getWindowManager().registerAframePkgNameInputChannel(mView.getContext().getPackageName(), mInputChannel);
				}
			}catch(Exception e){Log.d("ViewRootImpl","ViewRootImpl::draw(bool) - Error in registeration!!!!" + e);}

			mAframeContainerRect.offsetTo(mAframeContainerLocation[0],mAframeContainerLocation[1]);		
		}
		...
		int currUid = android.os.Process.myUid();
		int pairUid = -1;

		// pass the pair uid to gralloc (the current uid can be read by SurfaceTexture using Binder)
		//canvas = surface.lockCanvas(dirty);

		canvas = surface.lockCanvas(dirty, pairUid);
	...
	/////////////////////////////////////////////////////////////
	//AMIT - 04.22.2013
	//mActivityAframeBoundsMap is static

	private synchronized static void updateActivityAframeBoundsMap
					( String activityName, Rect aframeRect ) {

		mActivityAframeBoundsMap.put(activityName, aframeRect);
	...
	//XIAO 11/10/2012
	//Log.d("XIAO","------------ViewRootImpl---------------------------");							
	//Log.d("XIAO","Original MotionEvent in ViewRootImpl.java =  " + event);
	/*if(android.os.Process.myUid() == 10017){
		Matrix matrix = new Matrix();
		matrix.setTranslate(0,-280);
	    	event.transform(matrix);
	}*/
	//Log.d("XIAO","Transformed MotionEvent in ViewRootImpl.java =  " + event);
	//DONE 11/10/2012
	...
	//XIAO 11/10/2012
	//Log.d("XIAO","Is the event handled by the View Hierarchy?  " + handled);							
	//Log.d("XIAO","---------------ViewRootImpl Done----------------------------");
	//DONE 11/10/2012
