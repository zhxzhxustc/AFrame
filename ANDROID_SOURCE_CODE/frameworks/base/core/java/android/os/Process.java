...
//XIAO 12/08/2012
    private static final HashMap<Integer, Integer> mAframePidPair =
            new HashMap<Integer, Integer>();
    private static final HashMap<Integer, Integer> mAframePidPairReverse =
            new HashMap<Integer, Integer>();
//DONE 12/08/2012
...
/**
    ...
    public static final ProcessStartResult start(final String processClass,
                                  ...
                                  int uid, int gid, int[] gids,
                                  int debugFlags, int targetSdkVersion,
                                  String[] zygoteArgs, int pidPair) {
        try {
            ProcessStartResult result = startViaZygote(processClass, niceName, uid, gid, gids,
                    debugFlags, targetSdkVersion, zygoteArgs);
	    mAframePidPair.put(result.pid,pidPair);
	    mAframePidPairReverse.put(pidPair,result.pid);
	    Log.d("XIAO","Register Process Pair: [" + result.pid + ";" + pidPair + "]");
	    return result;
        } catch (ZygoteStartFailedEx ex) {
            Log.e(LOG_TAG,
                    "Starting VM process through Zygote failed");
...
//XIAO 12/12/2012
	Log.d("XIAO","About to killProcess pid = " + pid);
    	if(mAframePidPair.containsKey(pid)) {
	    int pidPair = mAframePidPair.get(pid);
	    mAframePidPair.remove(pid);	
	    if(mAframePidPairReverse.containsKey(pidPair))mAframePidPairReverse.remove(pidPair);
	    Log.d("XIAO","About to killProcess pid = " + pidPair);
	    sendSignal(pidPair, SIGNAL_KILL);
	} else if(mAframePidPairReverse.containsKey(pid)) {
	    int pidPair = mAframePidPairReverse.get(pid);
	    mAframePidPairReverse.remove(pid);	
	    if(mAframePidPair.containsKey(pidPair))mAframePidPair.remove(pidPair);
	    Log.d("XIAO","About to killProcess pid = " + pidPair);
	    sendSignal(pidPair, SIGNAL_KILL);
	}
//DONE 12/12/2012
