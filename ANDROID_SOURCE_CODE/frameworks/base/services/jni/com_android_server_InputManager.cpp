...
static void android_server_InputManager_nativeRegisterAppInputChannelRegion(JNIEnv* env, jclass clazz,
        jobject inputChannelObj, jobject regionObj) {
	//LOGD("XIAO: Inside nativeRegisterAppInputChannelRegion() in InputManager.cpp");
	const sp<InputChannel> inputChannel = android_view_InputChannel_getInputChannel(env,
            inputChannelObj);
	SkRegion *region = android_graphics_Region_getSkRegion(env, regionObj);
	gNativeInputManager->getInputManager()->getDispatcher()->nativeRegisterAppInputChannelRegion(inputChannel,*region);
...
static void android_server_InputManager_nativeRegisterAppInputChannelPkgName(JNIEnv* env, jclass clazz,
        jobject inputChannelObj, jstring pkgNameObj) {
	//LOGD("XIAO: Inside nativeRegisterAppInputChannelPkgName() in InputManager.cpp");
	const sp<InputChannel> inputChannel = android_view_InputChannel_getInputChannel(env,
            inputChannelObj);
	const char* pkgName = env->GetStringUTFChars(pkgNameObj, NULL);

	gNativeInputManager->getInputManager()->getDispatcher()->nativeRegisterAppInputChannelPkgName(inputChannel,pkgName);

	//Release local reference
	env->ReleaseStringUTFChars(pkgNameObj, pkgName);
	//env->DeleteLocalRef(inputChannel);
}
            

static void android_server_InputManager_nativeRegisterAframePkgNameInputChannel(JNIEnv* env, jclass clazz,
        jstring pkgNameObj, jobject inputChannelObj) {
	//LOGD("XIAO: Inside nativeRegisterAframePkgNameInputChannel() in InputManager.cpp");
	const sp<InputChannel> inputChannel = android_view_InputChannel_getInputChannel(env,
            inputChannelObj);
	const char* pkgName = env->GetStringUTFChars(pkgNameObj, NULL);
/*
	String8 pkgName("");
    	...
    	if (NULL != pkgNameObj && pkgNameObj != env->NewStringUTF("")) {
            char* bytes = const_cast< char* > (env->GetStringUTFChars(pkgNameObj, NULL));

            const int length = strlen(bytes) + 1;
            char *data = new char[length];
            strncpy(data, bytes, length);
            pkgName = String8(data);

            env->ReleaseStringUTFChars(pkgNameObj, bytes);
            delete [] data; data = NULL;
        }
*/
	gNativeInputManager->getInputManager()->getDispatcher()->nativeRegisterAframePkgNameInputChannel(pkgName,inputChannel);
...
static JNINativeMethod gInputManagerMethods[] = {
    ...
    //XIAO 11/30/2012
    { "nativeRegisterAppInputChannelRegion",
            "(Landroid/view/InputChannel;Landroid/graphics/Region;)V",
            (void*) android_server_InputManager_nativeRegisterAppInputChannelRegion },
    { "nativeRegisterAppInputChannelPkgName",
            "(Landroid/view/InputChannel;Ljava/lang/String;)V",
            (void*) android_server_InputManager_nativeRegisterAppInputChannelPkgName },
    { "nativeRegisterAframePkgNameInputChannel",
            "(Ljava/lang/String;Landroid/view/InputChannel;)V",
            (void*) android_server_InputManager_nativeRegisterAframePkgNameInputChannel },
    //DONE 11/30/2012
