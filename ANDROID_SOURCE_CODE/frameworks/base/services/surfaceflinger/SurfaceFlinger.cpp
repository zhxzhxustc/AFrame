...
//std::map<const char *, Rect> SurfaceFlinger::mActivityAframeBoundsMap;

static std::map<std::string, Rect> mActivityAframeBoundsMap;
...
//XIAO
//    const DisplayHardware& hw_copy(graphicPlane(0).displayHardware());
//    mDirtyRegion.set(hw_copy.bounds());
//DONE
...
//XIAO
/*	const char * aframeLayerName = "com.hello_word.com/com.hello_word.com.AframeActivity";	
	Region aframeRegion;
	const Rect aframeBounds(0,250,720,500);		
	aframeRegion.set(aframeBounds);
	if(layer->getName() == String8(aframeLayerName)){
	    screenRegion.set(aframeBounds);
	} else screenRegion.set(hw.bounds());
*/
...
//DONE
            ...
            if (!visibleRegion.isEmpty()) {
				...
				//////////////////////////////
				//AMIT - 04.18.2013
				//hard isolation change - 1 (total 4)

						
				// Algo:
				//
				// 1. check whether current layer's name exists as a key 
				// 	  in the mActivityAframeBoundsMap (layer has an aframe
				// 	  layer below it)
				//
				// 	  - use std::map::count(k) : searches the container for 
				// 	  elements with key equivalent to k and returns num of 
				// 	  matches; returns 1 for match, 0 otherwise
				//
				// 2. find the corresponding Rect dimensions from the map
				//    (use dimensions instead of the hard coded aframeBounds)
				//
				...
				const char * aframeLayerName = 
			"com.hello_word.com/com.hello_word.com.AframeActivity";
				...
				if( layer->getName() == String8(aframeLayerName) ) {
					...
					Rect aframeBounds;
					aframeBounds.left = 0;
					aframeBounds.top = 650;
					aframeBounds.right = 720;
					aframeBounds.bottom = 1184;
				*/

				const char* layerName = ( layer->getName() ).string();
				std::string str(layerName);

				if(mActivityAframeBoundsMap.count(str) > 0)
				{

					//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-1 : mActivityAframeBoundsMap.count(%s) > 0", layerName);

					//key exists
					Rect aframeBounds = mActivityAframeBoundsMap[str];

					//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-1 : aframeBounds - left(%d), top(%d), right(%d), bottom(%d)",  aframeBounds.left, aframeBounds.top, aframeBounds.right, aframeBounds.bottom);

		
					Region cutReg;
					cutReg.set(aframeBounds);

					// 1. subtract aframe bounds from visible reg
					visibleRegion.subtractSelf(cutReg);

					// 2. add aframe bounds to transparent reg
					(layer->transparentRegionScreen).orSelf(cutReg);

					// 3. change transprnt reg in layer drawing state
					Layer::State& s1(layer->currentState());
					s1.transparentRegion = layer->transparentRegionScreen;				  
		...
		//////////////////////////////
		//AMIT - 04.18.2013
		//hard isolation change - 2 (total 4)
		...
		*/
		...
		if(mActivityAframeBoundsMap.count(str) > 0)
		{
			//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-2 : mActivityAframeBoundsMap.count(%s) > 0",layerName);

			//key exists
			Rect aframeBounds = mActivityAframeBoundsMap[str];

			//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-2 : aframeBounds - left(%d), top(%d), right(%d), bottom(%d)",  aframeBounds.left, aframeBounds.top, aframeBounds.right, aframeBounds.bottom);

			Region cutReg;
			cutReg.set(aframeBounds);

			(layer->visibleRegionScreen).subtractSelf(cutReg);

			(layer->coveredRegionScreen).subtractSelf(cutReg);
		}
		...
		//////////////////////////////
		//AMIT - 04.18.2013
		//hard isolation change - 3 (total 4)
		...
		*/
		...
		if(mActivityAframeBoundsMap.count(str) > 0)
		{
			//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-3 : mActivityAframeBoundsMap.count(%s) > 0", layerName);

			//key exists
			Rect aframeBounds = mActivityAframeBoundsMap[str];

			//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-3 : aframeBounds - left(%d), top(%d), right(%d), bottom(%d)", aframeBounds.left, aframeBounds.top, aframeBounds.right, aframeBounds.bottom);

			Region cutReg;
			cutReg.set(aframeBounds);

			dirty.subtractSelf(cutReg);
		}
...
//XIAO	
//    LOGD("XIAO: opaqueRegion = [%d, %d] [%d, %d]",opaqueRegion.getBounds().left, opaqueRegion.getBounds().top,opaqueRegion.getBounds().right,opaqueRegion.getBounds().bottom);
//DONE
...
//XIAO		
//	LOGD("XIAO: mWormholeRegion = [%d, %d] [%d, %d]",mWormholeRegion.getBounds().left, mWormholeRegion.getBounds().top,mWormholeRegion.getBounds().right,mWormholeRegion.getBounds().bottom);
//DONE
...
//	mDirtyRegion.dump("mDirtyRegion",uint32_t(0));
...
void SurfaceFlinger::composeSurfaces(const Region& dirty)
{
	...
	// Algo:
	//
	// 1) iterate the vector to find if it contains top activity 
	// 	  layer
	// 	  - if yes, save layer names for found layer and previous
	// 	    layer in vector (layers in vector are bottom up)
	//		

	//char* aframeLayerName = NULL;
	//char* secondLayerName = NULL;


	
	std::string aframeLayerName;
	...
	// Iterate
 	for(size_t i=0; i<count; i++){
		...
		if(mActivityAframeBoundsMap.count(str) > 0)
		{
			...
			// Previous layer name
			const sp<LayerBase>& prevLayer(layers[i-1]);
			const char* second = ( prevLayer->getName() ).string();
			std::string str1(second);
			secondLayerName = str1;


			//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-4 (Iterate vector) : aframeLayerName = %s and secondLayerName = %s",aframeLayerName.c_str(), secondLayerName.c_str());
    ...
    for (size_t i=0 ; i<count ; i++) {
        ...
        if (cur && (cur[i].compositionType != HWC_FRAMEBUFFER)) {
			...
			////////////////////////////////////
			//AMIT - 04.18.2013
			//hard isolation change - 4 (total 4)
			...
			*/
			...
			/*		
		    const char * aframeLayerName = "com.hello_word.com/com.hello_word.com.AframeActivity";
		    const char * secondLayerName = "com.hello_word.com/com.hello_word.com.SecondActivity";

			const sp<LayerBase>& layer(layers[i]);

			if( (layer->getName() != String8(aframeLayerName)) && (layer->getName() != String8(secondLayerName))  ){
				continue;
			}
			*/
			 ...
			 * Algo:
			 *
			 * 1) if aframeLayerName and secondLayerName are NULL, then just continue
			 *
			 * 2) if both are NOT NULL, check if current layer name is NOT equal 
			 *    to either of them - continue only if true
			 *
			 */ 	

			
			if( aframeLayerName.empty() && secondLayerName.empty() ){

				//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-4 : aframeLayerName & secondLayerName empty");

				continue;
			}
			else if( !aframeLayerName.empty() && !secondLayerName.empty() ){
	
				//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-4 : aframeLayerName & secondLayerName NON-empty");
				...
				// Current layer should not be aframe layer and second layer
				if( (str.compare(aframeLayerName) != 0) &&
					(str.compare(secondLayerName) != 0)
				  )
				{

					//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-4 : current layer NOT aframeLayer AND secondLayer");
				...
				//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::HrdIso-4 : aframeLayerName & secondLayerName - else block");
...
/*	const char * aframeLayerName = "com.hello_word.com/com.hello_word.com.AframeActivity";
	if(layer->getName() == String8(aframeLayerName)){		
		LOGD("XIAO: AFRAME CLIP REGION = [%d, %d] [%d, %d]",clip.getBounds().left, clip.getBounds().top,clip.getBounds().right,clip.getBounds().bottom);
	}

    	const char * secondLayerName = "com.hello_word.com/com.hello_word.com.SecondActivity";
	if(layer->getName() == String8(secondLayerName)){		
		LOGD("XIAO: SECOND CLIP REGION = [%d, %d] [%d, %d]",clip.getBounds().left, clip.getBounds().top,clip.getBounds().right,clip.getBounds().bottom);
	}
*/
//DONE
        if (!clip.isEmpty()) {		
...
bool SurfaceFlinger::hasAframe(const String8& layerName){
    
    Mutex::Autolock _l(mAframeLock);
	const char* name = layerName.string();
    std::string str(name);
    if(mActivityAframeBoundsMap.count(str) > 0) return true;
    else return false;
}

void SurfaceFlinger::saveAframeBounds(const String8& activityName, 
		   int left, int top, int right, int bottom)
{
    Mutex::Autolock _l(mAframeLock);

	// Activity name -> std::string for key
	const char* name = activityName.string();
	std::string str(name);

	//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::saveAframeBounds : arg - activityName(%s), left(%d), top(%d), right(%d), bottom(%d)", name, left, top, right, bottom);

	// Value Rect	
	Rect r(left, top, right, bottom);

	// key is std::string, value is Rect
	mActivityAframeBoundsMap[str] = r;

	//LOGD("AMIT - POSTBOUNDS - SurfaceFlinger::saveAframeBounds : mActivityAframeBoundsMap[%s] = [left = %d, top = %d, right = %d, bottom = %d]", str.c_str(), mActivityAframeBoundsMap[str].left, mActivityAframeBoundsMap[str].top, mActivityAframeBoundsMap[str].right, mActivityAframeBoundsMap[str].bottom);
...
//////////////////////////////////////////////////////////////////
//AMIT - added overload with uid argument + pairUid argument
sp<GraphicBuffer> GraphicBufferAlloc::createGraphicBuffer(uint32_t w, uint32_t h,
        PixelFormat format, uint32_t usage, status_t* error, uint32_t uid, uint32_t pairUid) {

    // overload ctor uses uid
    sp<GraphicBuffer> graphicBuffer(new GraphicBuffer(w, h, format, usage, uid, pairUid));
    status_t err = graphicBuffer->initCheck();
    *error = err;
    if (err != 0 || graphicBuffer->handle == 0) {
        if (err == NO_MEMORY) {
