...
//XIAO 12/01/2012
//static std::map<String8, const SkRegion*> mInputChannelRegionMap;
static std::map<String8, SkRegion> mInputChannelRegionMap;
static std::map<String8, String8> mInputChannelPkgNameMap;
static std::map<String8, String8> mPkgNameInputChannelMap;
//DONE 12/01/2012
...
//XIAO 11/03/2012
    LOGD("XIAO: Starting Dispatching Event time in nanoSecond = %lld", now());
    bool Channel_Name_Compare_Result = false;
    bool Event_AXIS_Compare_Result = false;
    float left = 0.0;
    float right = 0.0;
    float top = 0.0;
    float bottom = 0.0;
    char pkgName[200];
    static bool focusInSecond = false;

    for (size_t i = 0; i < mCurrentInputTargets.size(); i++) {
        const InputTarget& inputTarget = mCurrentInputTargets.itemAt(i);
	
	char tempChannelName[200];
	strcpy(tempChannelName,inputTarget.inputChannel->getName().string());
	tempChannelName[strlen(tempChannelName)-9] = '\0';
	//LOGD("XIAO: TEXT = %s",tempChannelName);

	if(mInputChannelRegionMap.count(String8(tempChannelName)) > 0){
	    Channel_Name_Compare_Result = true;
    	    //LOGD("XIAO: Channel_Name_Compare_Result = true");
	    //LOGD("XIAO: Count = %d",mInputChannelPkgNameMap.count(String8(tempChannelName)));
	    strcpy(pkgName, mInputChannelPkgNameMap[String8(tempChannelName)].string());
	    //LOGD("XIAO: pkgName = %s",pkgName);

	    const SkRegion& mapRegion = (mInputChannelRegionMap[String8(tempChannelName)]);

	    for (SkRegion::Iterator it(mapRegion); !it.done(); it.next()) {
		const SkIRect& rect = it.rect();
		left = rect.fLeft;
		right = rect.fRight;
		top = rect.fTop;
		bottom = rect.fBottom;
        	//LOGD("XIAO: channel '%s' ~ dispatchEventToCurrentInputTargetsLocked - [%d,%d][%d,%d]", inputTarget.inputChannel->getName().string(), rect.fLeft, rect.fTop, rect.fRight, rect.fBottom);
    	    }
    }

    	if(!Channel_Name_Compare_Result)continue;

    	if (eventEntry->type == EventEntry::TYPE_MOTION) {
            MotionEntry* motionEntry = static_cast<MotionEntry*>(eventEntry);
            MotionSample* sample = & motionEntry->firstSample;
            for (; sample->next != NULL; sample = sample->next) {
            }
	    //LOGD("XIAO: MotionEvent y ='%f' .",sample->pointerCoords[0].getAxisValue(AMOTION_EVENT_AXIS_Y));
	    Event_AXIS_Compare_Result = (sample->pointerCoords[0].getAxisValue(AMOTION_EVENT_AXIS_X) >= left && 
					sample->pointerCoords[0].getAxisValue(AMOTION_EVENT_AXIS_X) <= right &&
					sample->pointerCoords[0].getAxisValue(AMOTION_EVENT_AXIS_Y) >= top &&
					sample->pointerCoords[0].getAxisValue(AMOTION_EVENT_AXIS_Y) <= bottom);
	    if(Event_AXIS_Compare_Result)focusInSecond = true;
	    else focusInSecond = false;
        }

        if(Event_AXIS_Compare_Result)break;
    }

    sp<InputWindowHandle> windowHandleCopy = mFocusedWindowHandle;
 
    if(Channel_Name_Compare_Result && (Event_AXIS_Compare_Result || focusInSecond)){

	// clear the current input targets list
	mCurrentInputTargets.clear();
	size_t numWindows = mWindowHandles.size();
        for (size_t i = 0; i < numWindows; i++) {
            sp<InputWindowHandle> windowHandle = mWindowHandles.itemAt(i);
	    const char* channelNameWithExtras = windowHandle->getInfo()->inputChannel->getName().string();
	
	    char tempChannelName[200];
    	    strcpy(tempChannelName,channelNameWithExtras);
	    tempChannelName[strlen(tempChannelName)-9] = '\0';

	    // Found the registed Aframe Input Target
	    if(mPkgNameInputChannelMap.count(String8(pkgName)) > 0 ){
		//LOGD("XIAO: tempChannelName: '%s' .",tempChannelName);
  	        //LOGD("XIAO: mPkgNameInputChannelMap[pkgName]: '%s' .",mPkgNameInputChannelMap[String8(pkgName)].string());

		if(mPkgNameInputChannelMap[String8(pkgName)]==String8(tempChannelName)){
			//LOGD("XIAO: FOUND IT!!!!!");
			//Add the target to current input targets list
			windowHandleCopy = windowHandle;
			addWindowTargetLocked(windowHandle,
            			InputTarget::FLAG_DISPATCH_AS_IS | InputTarget::FLAG_FOREGROUND | InputTarget::FLAG_WINDOW_IS_OBSCURED, BitSet32(0));
			break;
		}
	    }
...
/*    if (eventEntry->isInjected()){
        LOGW("Injecting event from pid %d uid %d to window %s " "owned by uid %d",
                eventEntry->injectionState->injectorPid, eventEntry->injectionState->injectorUid,
                windowHandleCopy->getName().string(),
                windowHandleCopy->getInfo()->ownerUid);
    }
*/
    if (eventEntry->isInjected() && (windowHandleCopy == NULL
        || windowHandleCopy->getInfo()->ownerUid != eventEntry->injectionState->injectorUid)
	&& !hasInjectionPermission(eventEntry->injectionState->injectorPid, eventEntry->injectionState->injectorUid)) {
	LOGW("Permission denied: injecting event from pid %d uid %d to window %s " "owned by uid %d",
            eventEntry->injectionState->injectorPid, eventEntry->injectionState->injectorUid,
            windowHandleCopy->getName().string(),
            windowHandleCopy->getInfo()->ownerUid);
	return;
    ...
    //mFocusedWindowHandle = windowHandleCopy;
    LOGD("Decision Made at time in nanoSecond = %lld", now());
//DONE 11/03/2012
...
bool InputDispatcher::checkInjectionPermission(const sp<InputWindowHandle>& windowHandle,
    ...
    if (injectionState
	    ...
	    //XIAO 01/08/2013
    	    ...
    	    strcpy(tempChannelName,windowHandle->getInputChannel()->getName().string());
    	    tempChannelName[strlen(tempChannelName)-9] = '\0';
 	    if(mInputChannelRegionMap.count(String8(tempChannelName)) > 0)return true;
...
void InputDispatcher::nativeRegisterAppInputChannelRegion(const sp<InputChannel>& inputChannel,const SkRegion& region){
	char tempName[200];
	strcpy(tempName,inputChannel->getName().string());
	tempName[strlen(tempName)-9] = '\0';
	//LOGD("nativeRegisterAppInputChannelRegion TEXT = %s",tempName);

	String8 channelName(tempName);

	mInputChannelRegionMap[channelName] = region;
	const SkRegion& mapRegion = (mInputChannelRegionMap[channelName]);
	for (SkRegion::Iterator it(mapRegion); !it.done(); it.next()) {
            const SkIRect& rect = it.rect();
            //LOGD("XIAO: channel '%s' ~ nativeRegisterAppInputChannelRegion - [%d,%d][%d,%d]", inputChannel->getName().string(), rect.fLeft, rect.fTop, rect.fRight, rect.fBottom);
    	}
		
}

void InputDispatcher::nativeRegisterAppInputChannelPkgName(const sp<InputChannel>& inputChannel, const char* pkgName){
	char tempName[200];
	strcpy(tempName,inputChannel->getName().string());
	tempName[strlen(tempName)-9] = '\0';
	//LOGD("nativeRegisterAppInputChannelRegion TEXT = %s",tempName);
	...
	String8 channelName(tempName);
	String8 packageName(pkgName);

	mInputChannelPkgNameMap[channelName] = packageName;
	//LOGD("XIAO: channel '%s' ~ nativeRegisterAppInputChannelPkgName - pkgName=%s", inputChannel->getName().string(),pkgName);
}

void InputDispatcher::nativeRegisterAframePkgNameInputChannel(const char* pkgName, const sp<InputChannel>& inputChannel){
	char tempName[200];
	strcpy(tempName,inputChannel->getName().string());
	tempName[strlen(tempName)-9] = '\0';
	//LOGD("nativeRegisterAppInputChannelRegion TEXT = %s",tempName);
	...
	mPkgNameInputChannelMap[packageName] = channelName;
	//LOGD("XIAO: channel '%s' ~ nativeRegisterAframePkgNameInputChannel - pkgName=%s", inputChannel->getName().string(),pkgName);
}
