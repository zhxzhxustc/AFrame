...
final class ActivityStack {
    ...
    static final boolean DEBUG_PAUSE = true;
...
//XIAO 12/08/2012
    final HashMap<IBinder, IBinder> mAframeIBinderPair =
            new HashMap<IBinder, IBinder>();
    final HashMap<IBinder, IBinder> mAframeIBinderPairReverse =
            new HashMap<IBinder, IBinder>();
//DONE 12/08/2012
...
//XIAO 12/08/2012
     ...
     private final void startPausingAframeLocked(ActivityRecord record, boolean userLeaving, boolean uiSleeping) {
    ...
    final void addAframeIBinderTokenPair(IBinder aframeActivity, IBinder mainActivity) {
	    mAframeIBinderPair.put(aframeActivity,mainActivity);
	    mAframeIBinderPairReverse.put(mainActivity,aframeActivity);
    }
...
//XIAO
	IBinder tempToken = null;

        synchronized (mService) {
            int index = indexOfTokenLocked(token);
//XIAO 12/08/2012
            if (index >= 0 || mAframeIBinderPair.containsKey(token)) {
		if(index >= 0)
...
//XIAO
		    tempToken = mPausingActivity.appToken;
...
//XIAO 12/08/2012
	//Log.d("XIAO","mAframeIBinderPairReverse.get(tempToken) = " + mAframeIBinderPairReverse.get(tempToken));
	//Log.d("XIAO","mAframeIBinderPair.get(tempToken) = " + mAframeIBinderPair.get(tempToken));
	if(mAframeIBinderPair.containsKey(tempToken)){
	    //Log.d("XIAO","---------- DEBUGGING TOKEN -------");
	    IBinder sync_token = mAframeIBinderPair.get(tempToken);
	    //Log.d("XIAO","sync_token = " + sync_token);
	    ActivityRecord sync_record = ActivityRecord.forToken(sync_token);
	    //Log.d("XIAO","sync_record = " + sync_record);
	    if(sync_record!=null && sync_record.state!=ActivityState.PAUSING && sync_record.state!=ActivityState.PAUSED) {
		startPausingAframeLocked(sync_record,mUserLeaving, false);
	    }
	} else if(mAframeIBinderPairReverse.containsKey(tempToken)){
	    //Log.d("XIAO","---------- DEBUGGING TOKEN -------");
	    IBinder sync_token = mAframeIBinderPairReverse.get(tempToken);
	    //Log.d("XIAO","sync_token = " + sync_token);
	    ActivityRecord sync_record = ActivityRecord.forToken(sync_token);
	    //Log.d("XIAO","sync_record = " + sync_record);
	    if(sync_record!=null && sync_record.state!=ActivityState.PAUSING && sync_record.state!=ActivityState.PAUSED) {
...
//DONE 12/08/2012
	...
	//Log.d("XIAO","Inside activityStoppedLocked(): r = " + r);
	IBinder tempToken = r.appToken;
...
//XIAO 12/08/2012
	...
	if(mAframeIBinderPair.containsKey(tempToken)){
	    ...
	    ActivityRecord sync_record = ActivityRecord.forToken(sync_token);
	    //Log.d("XIAO","sync_record = " + sync_record);
	    if(sync_record!=null && sync_record.state!=ActivityState.STOPPED && sync_record.state!=ActivityState.STOPPING) {
		stopActivityLocked(sync_record);
	    }
	} else if(mAframeIBinderPairReverse.containsKey(tempToken)){
	    //Log.d("XIAO","---------- DEBUGGING TOKEN -------");
	    IBinder sync_token = mAframeIBinderPairReverse.get(tempToken);
...
//DONE 12/08/2012
	...
	//Log.d("XIAO","Inside resumeTopActivityLocked()");
	...
	//XIAO 12/08/2012
	//Log.d("XIAO","mAframeIBinderPairReverse.get(next.appToken) = " + mAframeIBinderPairReverse.get(next.appToken));
	//Log.d("XIAO","mAframeIBinderPair.get(next.appToken) = " + mAframeIBinderPair.get(next.appToken));
		if(mAframeIBinderPair.containsKey(next.appToken)){
	   	    //Log.d("XIAO","---------- DEBUGGING TOKEN ONRESUME-------");

	    	    IBinder sync_token = mAframeIBinderPair.get(next.appToken);
	    	    //Log.d("XIAO","sync_token = " + sync_token);
		    
	    	    ActivityRecord sync_record = ActivityRecord.forToken(sync_token);
	    	    //Log.d("XIAO","sync_record = " + sync_record);

	    	    if(sync_record!=null && sync_record.state!=ActivityState.RESUMED) {
			//Log.d("XIAO","About to resume ActivityRecord = " + sync_record);
			mService.mWindowManager.setAppVisibility(sync_token, true);
		    	sync_record.state = ActivityState.RESUMED;
			sync_record.task.touchActiveTime();
			sync_record.sleeping = false;
                	showAskCompatModeDialogLocked(sync_record);
                	sync_record.app.pendingUiClean = true;
                	sync_record.app.thread.scheduleResumeActivity(sync_record.appToken,
                        	mService.isNextTransitionForward());
	    	    }
	   	    //Log.d("XIAO","---------- DEBUGGING TOKEN ONRESUME DONE-------");

		} else if(mAframeIBinderPairReverse.containsKey(next.appToken)){
	   	    //Log.d("XIAO","---------- DEBUGGING TOKEN ONRESUME-------");

	    	    IBinder sync_token = mAframeIBinderPairReverse.get(next.appToken);
	    	    //Log.d("XIAO","sync_token = " + sync_token);
	...
	Log.d("TTS","Ending Launching Time for process <" + r + "> in nanoSecond: " +  System.nanoTime());
	...
	//Log.d("XIAO","Inside stopActivityLocked(): r = " + r);
	...
	//XIAO 12/11/2012
		if(mAframeIBinderPair.containsKey(r.appToken)){
		    r.visible = false;
		}
	//DONE 12/11/2012
    ...
    private final ActivityRecord finishCurrentActivityLocked(ActivityRecord r,
        ...
        if (mode == FINISH_IMMEDIATELY
            ...
            // it right now.		destroyActivityLocked(r, true, false, "finish-idle");
    ...
    final void cleanUpActivityLocked(ActivityRecord r, boolean cleanServices,
        ...
        if (mService.mPendingThumbnails.size() > 0) {
            // There are clients waiting to receive thumbnails so, in case		destroyActivityLocked(r, true, false, "finish-idle");
	...
	//Log.d("XIAO","Inside destroyActivityLocked(): r = " + r);
		...
		//Log.d("XIAO","About to destroy r = " + r);

                r.app.thread.scheduleDestroyActivity(r.appToken, r.finishing,
                        r.configChangeFlags);
            } catch (Exception e) {

		//Log.d("XIAO","Exception in destroying activityRecord r");
...
//XIAO
	//Log.d("XIAO","Inside activityDestroyed(): token " + token);
	IBinder tempToken = token;
...
//XIAO 12/08/2012
	...
	if(mAframeIBinderPair.containsKey(tempToken)){
	    ...
	    ActivityRecord sync_record = ActivityRecord.forToken(sync_token);
	    //Log.d("XIAO","sync_record = " + sync_record);
	    if(sync_record!=null && sync_record.state!=ActivityState.DESTROYING && sync_record.state!=ActivityState.DESTROYED) {
		destroyActivityLocked(sync_record, true, false, "finish-idle");
	    }
	} else if(mAframeIBinderPairReverse.containsKey(tempToken)){
	    //Log.d("XIAO","---------- DEBUGGING TOKEN -------");
	    IBinder sync_token = mAframeIBinderPairReverse.get(tempToken);
