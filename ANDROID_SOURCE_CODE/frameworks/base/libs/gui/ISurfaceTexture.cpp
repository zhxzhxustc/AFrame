...
public:
    ...
    /////////////////////////////////////////////////////
    // AMIT - added overload with pairUid argument
    ...
    virtual status_t dequeueBuffer(int *buf, uint32_t w, uint32_t h,
        ...
        // AMIT
        data.writeInt32(pairUid);

        status_t result = remote()->transact(DEQUEUE_BUFFER2, data, &reply);
        if (result != NO_ERROR) {
            return result;
        }
        *buf = reply.readInt32();
	...
	virtual void postAframeBounds(const String8& activityName,
			uint32_t left, uint32_t top, uint32_t right, uint32_t bottom)
	{
		Parcel data, reply;
		data.writeInterfaceToken(ISurfaceTexture::getInterfaceDescriptor());
		data.writeString8(activityName);
        data.writeInt32(left);
        data.writeInt32(top);
        data.writeInt32(right);
        data.writeInt32(bottom);

		//LOGD("AMIT - POSTBOUNDS - ISurfaceTexture - postAframeBounds (write): arg - activityName(%s), left(%u), top(%u), right(%u), bottom(%u)", activityName.string(), left, top, right, bottom);


		remote()->transact(POST_AFRAME_BOUNDS, data, &reply);
	}
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
	...
	////////////////////////////////////////////
	    ...
	    // AMIT
            uint32_t pairUid  = data.readInt32();
            // DONE

            int buf;
            int result = dequeueBuffer(&buf, w, h, format, usage, pairUid);
            reply->writeInt32(buf);
            reply->writeInt32(result);
            return NO_ERROR;
        } break;
		...
		case POST_AFRAME_BOUNDS: {
			
			CHECK_INTERFACE(ISurfaceTexture, data, reply);
			String8 activityName = data.readString8();
			uint32_t left = data.readInt32();
			uint32_t top = data.readInt32();
			uint32_t right = data.readInt32();
			uint32_t bottom = data.readInt32();

			//LOGD("AMIT - POSTBOUNDS - ISurfaceTexture - postAframeBounds (read): arg - activityName(%s), left(%u), top(%u), right(%u), bottom(%u)", activityName.string(), left, top, right, bottom);


			postAframeBounds(activityName, 
					left, top, right, bottom);

			//LOGD("AMIT - POSTBOUNDS - ISurfaceTexture - after postAframeBounds() ");
