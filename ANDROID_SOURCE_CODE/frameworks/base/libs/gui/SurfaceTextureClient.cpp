...
int SurfaceTextureClient::hook_perform(ANativeWindow* window, int operation, ...) {
	...
	//AMIT
	//LOGD("AMIT - SurfaceTextureClient::hook_perform(window, operation = %d, ...) invoked", operation);
	//DONE
...
int SurfaceTextureClient::dequeueBuffer(android_native_buffer_t** buffer) {
    ...
    /*if(uid == 10017 || uid == 10034){
	LOGE("AMIT - SurfaceTextureClient - dequeueBuffer - uid(%d) slot index = %d",uid, buf); 
	LOGE("AMIT - SurfaceTextureClient - dequeueBuffer - uid(%d) result = %d",uid, result);
    }*/
    //DONE
    ...
    /*
    if(getuid() == 10017){
	    LOGE("AMIT - SurfaceTextureClient::deQBuffer - uid(10017) - before setting width 100");
	    gbuf->width = 100;
	    LOGE("AMIT - SurfaceTextureClient::deQBuffer - uid(10017) - after setting width 100");
    }
    */
    //DONE
...
////////////////////////////////////////////////
// AMIT - overload with uid argument

int SurfaceTextureClient::dequeueBuffer(android_native_buffer_t** buffer, uint32_t pairUid) {
    LOGV("SurfaceTextureClient::dequeueBuffer");
    Mutex::Autolock lock(mMutex);
    int buf = -1;
	...
	//AMIT
	//LOGD("AMIT - SurfaceTextureClient::dequeueBuffer() - mReqWidth = %u, mReqHeight = %u", mReqWidth , mReqHeight);
	//DONE


    ///////////////////////////////////////
    // AMIT - added pair uid argument

    status_t result = mSurfaceTexture->dequeueBuffer(&buf, mReqWidth, mReqHeight,
            mReqFormat, mReqUsage, pairUid);
...
int SurfaceTextureClient::queueBuffer(android_native_buffer_t* buffer) {
    ...
    //AMIT-- Comment out By XIAO, save the terminal space
    //LOGE("AMIT - SurfaceTextureClient::queueBuffer- uid=%d",getuid());
    //DONE
...
{
	//AMIT
	//LOGE("AMIT - SurfaceTextureClient::perform(operation = %d, args) invoked", operation);
	//DONE
    ...
    case NATIVE_WINDOW_SET_BUFFERS_TRANSFORM:
		...
		//AMIT
		//LOGE("AMIT - case : NATIVE_WINDOW_SET_BUFFERS_TRANSFORM");
		//DONE
    ...
    case NATIVE_WINDOW_SET_BUFFERS_DIMENSIONS:
		...
		//AMIT
		//LOGE("AMIT - case : NATIVE_WINDOW_SET_BUFFERS_DIMENSIONS");
		//DONE
...
int SurfaceTextureClient::dispatchSetBuffersGeometry(va_list args) {
	...
	//AMIT
	//LOGE("AMIT - SurfaceTextureClient::dispatchSetBuffersGeometry() invoked");	
	//DONE
...
int SurfaceTextureClient::dispatchSetBuffersDimensions(va_list args) {
	...
	//AMIT
	//LOGE("AMIT - SurfaceTextureClient::dispatchSetBuffersDimensions() invoked");	
	//DONE
...
int SurfaceTextureClient::dispatchSetBuffersTransform(va_list args) {
	...
	//AMIT
	//LOGE("AMIT - SurfaceTextureClient::dispatchSetBuffersTransform() invoked");	
	//DONE
...
int SurfaceTextureClient::setBuffersDimensions(int w, int h)
{
	...
	//AMIT
	//LOGE("AMIT - SurfaceTextureClient::setBuffersDimension(w = %d,h = %d) invoked", w, h);
	//DONE
        ...
        ANativeWindow_Buffer* outBuffer, ARect* inOutDirtyBounds)
{
    ...
    //LOGE("AMIT - SurfaceTextureClient::lock called for uid=%d", getuid());
    ...
    //AMIT
    //LOGE("AMIT - 1 - SurfaceTextureClient::lock - after dequeue buffer");
    //DONE
	...
	//AMIT
   	//LOGE("AMIT - 2 - SurfaceTextureClient::lock - no error in dequeue buffer step");
    	//DONE

        sp<GraphicBuffer> backBuffer(GraphicBuffer::getSelf(out));

/*
	void* p = (void*)out->handle;
	LOGE("AMIT - 2.0.1 - out->handle");
	private_handle_t* bufferHandle = (private_handle_t*)p;
	LOGE("AMIT - 2.0.1 - type cast to private_handle_t");
	LOGE("AMIT - 2.0.2 - fd after dequeue buffer = %d", bufferHandle->fd);
*/
	...
	//AMIT
   	//LOGE("AMIT - 3 - SurfaceTextureClient::lock - after lock buffer step");
	//LOGE("AMIT - 3.1 - SurfaceTextureClient::lock - uid(%d), buffer width = %d", getuid(), backBuffer->width);
    	//DONE


	//AMIT
        //LOGE("AMIT - 3.1 - SurfaceTextureClient::lock - buffer width = %d", backBuffer->width);	// 320
	//LOGE("AMIT - 3.2 - SurfaceTextureClient::lock - buffer height = %d", backBuffer->height); // 480

        /*
	static int w = -1;
	if(getuid() == 10017){
		LOGE("AMIT - before copied width");
		w = backBuffer->width;
		LOGE("AMIT - copied width = %d",w);
	}
        */
	...
	if (err == NO_ERROR) {
	    ...
	    Region newDirtyRegion;
            ...
            //AMIT
            //LOGE("AMIT - SurfaceTextureClient::lock - before assigning mapped fd pointer");
	    //LOGE("AMIT - backbuffer stride : %d", backBuffer->stride); // 320
	    //LOGE("AMIT - backbuffer format : %d", backBuffer->format); // 1
	    //DONE
	...
	if(getuid() == 10017){
	    ...
	    outBuffer->width  = 320; //backBuffer->width;
            outBuffer->height = 480; //backBuffer->height;
            outBuffer->stride = 320; //backBuffer->stride;
            outBuffer->format = 1; //backBuffer->format;
            outBuffer->bits   = vaddr;
...
status_t SurfaceTextureClient::lock(
        ANativeWindow_Buffer* outBuffer, ARect* inOutDirtyBounds, int pairUid)
{
    ...
    ANativeWindowBuffer* out;
    status_t err = dequeueBuffer(&out, pairUid);
...
void SurfaceTextureClient::postAframeBounds(const String8& activityName,
		int left, int top, int right, int bottom){

	//LOGD("AMIT - POSTBOUNDS - SurfaceTextureClient - postAframeBounds : arg - activityName(%s), left(%d), top(%d), right(%d), bottom(%d)",activityName.string(), left, top, right, bottom);

	mSurfaceTexture->postAframeBounds(activityName, left, top, right, bottom);
}
...
status_t SurfaceTextureClient::unlockAndPost()
{
    ...
    //AMIT
    //LOGE("SurfaceTextureClient-unlockAndPost-after buffer unlock");
    ...
    //AMIT
    //LOGE("SurfaceTextureClient-unlockAndPost-after queue buffer");
