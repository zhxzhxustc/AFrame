...
//////////////////////////////////////////////////////
// AMIT - overload ctor with uid arg + pairUid
GraphicBuffer::GraphicBuffer(uint32_t w, uint32_t h, 
        PixelFormat reqFormat, uint32_t reqUsage, uint32_t uid, uint32_t pairUid)
    : BASE(), mOwner(ownData), mBufferMapper(GraphicBufferMapper::get()),
      mInitCheck(NO_ERROR), mIndex(-1)
{
    width  = 
    ...
    usage  = 0;
    handle = NULL;
    mInitCheck = initSize(w, h, reqFormat, reqUsage, uid, pairUid);
}
...
/////////////////////////////////////
// AMIT - overload with uid + pairUid
status_t GraphicBuffer::initSize(uint32_t w, uint32_t h, PixelFormat format,
        uint32_t reqUsage, uint32_t uid, uint32_t pairUid)
{
    GraphicBufferAllocator& allocator = GraphicBufferAllocator::get();
    status_t err = allocator.alloc(w, h, format, reqUsage, &handle, &stride, uid, pairUid);
    if (err == NO_ERROR) {
        this->width  = w;
        this->height = h;
        this->format = format;
        ...
        int fds[], size_t count)
{
    ...
    //XIAO: 03/05/2013
    LOGD("Inside flatten():width=%d,height=%d,size=%d",buf[1],buf[2],size);
    //DONE: 03/05/2013
